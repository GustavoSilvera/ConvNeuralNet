...
const size_t net::get_diff() const
{
  return diff;
}
layer &net::get_layer(size_t i)
{
  if (i >= layers.size())
    printf("BAD LAYER %zu %zu %zu", i, layers.size(), num_layers);
  return layers[i];
}
...
void net::init(std::vector<vec2> &pos, std::vector<std::vector<double>> &total_data, std::vector<size_t> &num_inputs)
{
  position = pos; //updates local position matrix
  for (size_t i = 0; i < num_layers; i++)
  {
    //creates the layers of the network
    layers.emplace_back(num_neurons[i], position[i]);
  }
  ...
  { //for every (behind) layer [not the last one]
    std::vector<std::vector<double>> ind_neuron;
    for (size_t j = 0; j < layers[i].get_num_neurons(); j++)
    { //for every neuron in said layer
  ...
  { //for every later layer (not the first one)
...
void net::extract_data(std::vector<std::vector<double>> &total_data, std::vector<size_t> &num_inputs)
{
  for (size_t i = 0; i < total_data_lines; i++)
  {
    ...
    { //num_inputs[i] has index for all data (lines') individual num_inputs (should all be same)
      ind_line_data.push_back(total_data[i][j]);
    }
    for (size_t j = num_inputs[i]; j < total_data[j].size(); j++)
    { //scans through all (OUTPUT) elements in total_data set
      if (j - num_inputs[i] == focus_variable)
...
{
  if (data_line >= total_data_lines)
    data_line = 0; //resets after a cycle
  for (size_t i = 1; i < num_layers; i++)
  { //only the next layers (last ones)
    std::vector<double> new_values;
    const size_t layer_num_neurons = layers[i].get_num_neurons();
    for (size_t j = 0; j < layer_num_neurons; j++)
    { //for every neuron in the next layers
      double weighted_sum = 0;
      const size_t prev_layer_num_neurons = layers[i - 1].get_num_neurons();
      for (size_t k = 0; k < prev_layer_num_neurons; k++)
      { //back to every neuron in last layer
      ...
      {
        if (using_sigmoid)
          new_values.push_back(sigmoid(weighted_sum + bias[i - 1][j]));
        else
          new_values.push_back(smooth_RelU(weighted_sum + bias[i - 1][j]));
      }
      else
      {
...
void net::test_data(layer *optimal)
{
  ...
  for (size_t i = 0; i < layers[0].get_num_neurons(); i++)
  { //first .num_neuron elements in data[] is inputs
  ...
  for (size_t i = layers[0].get_num_neurons(); i < data[data_line].size(); i++)
  { //starting where last loop left off
    new_outputs.push_back(data[data_line][i]);
  }
  optimal->set_weights(new_outputs); //optimal (the "should be" output) layer is second
...
void net::randomize_weights()
{
  ...
  { //for every layer (except last one)
    ...
    { //for every neuron in said layer
      for (size_t k = 0; k < layers[i + 1].get_num_neurons(); k++)
      {                                             //for every weight its attached to
...
net::total_changes net::improve(int i, layer *ideal, std::vector<double> &t_changes)
{
  ...
  if (t_changes.size() == 0)
  { //basically only for the output layer (same dimens as output-ideal)
    for (size_t j = 0; j < layers[i].get_num_neurons(); j++)
    {
  ...
  { //for every neuron in said layer
    ...
    for (size_t k = 0; k < layers[i - 1].get_num_neurons(); k++)
    {                                                                                        //for every (behind)weight its attached to
...
{
  if (t_changes.size() == 0)
    layerInd = num_layers - 1; //very first condition
  if (layerInd > 0)
  { //recursion failsafe (exit condition)
    ...
    for (size_t line = 0; line < num_changes; line++)
    { //obtain all the desired modifications from every data set
      new_data(rel_ideal);
      changes.push_back(improve(layerInd, ideal, t_changes));
    }
    for (size_t i = 0; i < layers[layerInd].get_num_neurons(); i++)
    { //all neurons
      double avg_bias = 0;
    ...
    for (size_t i = 0; i < layers[layerInd - 1].get_num_neurons(); i++)
    { //all neurons in last layer(weights are emitted)
      for (size_t j = 0; j < layers[layerInd].get_num_neurons(); j++)
      { //for the individual weight of said neuron
        double avg_weight = 0;
        for (size_t k = 0; k < num_changes; k++)
        {                                                //all data values
    ...
    comp_avg_cost(rel_ideal); //computes total average cost of thee entire net
}
...
string net::output() const
{
  ...
  for (size_t i = 0; i < num_layers; i++)
  {
    ...
    if (i < num_layers - 1)
      ret.append(", "); //comma to separate the num_neurons
  }
  ret.append("}; \n");
  //print all weights
  ...
  for (size_t i = 0; i < num_layers - 1; i++)
  {
    ...
    for (size_t j = 0; j < layers[i].get_num_neurons(); j++)
    {
      ...
      for (size_t k = 0; k < layers[i + 1].get_num_neurons(); k++)
      {
        ...
        if (k < layers[i + 1].get_num_neurons() - 1)
          ret.append(", "); //comma to separate the weights
      }
      ret.append("}"); //close neuron
      if (j < layers[i].get_num_neurons() - 1)
        ret.append(", \n");
      else
        ret.append("\n");
    }
  ...
  for (size_t i = 0; i < num_layers - 1; i++)
  {
    ...
    for (size_t j = 0; j < layers[i + 1].get_num_neurons(); j++)
    {
      ...
      if (j < layers[i + 1].get_num_neurons() - 1)
        ret.append(", "); //comma to separate the biases
    }
...
void net::resize(const size_t w, const size_t h)
{
  ...
  { //only the next layers (last ones)
    ...
    { //for every neuron in the next layers
      ...
      for (size_t k = 0; k < prev_layer_num_neurons; k++)
      {                                                                                  //back to every neuron in last layer
        weighted_sum += layers[i - 1].get_neuron(k).get_weight() * weights[i - 1][k][j]; //adds to the weighted sum
      }
      if (i < layers[i].get_num_neurons() - 1)
      {
        if (using_sigmoid)
          new_values.push_back(sigmoid(weighted_sum + bias[i - 1][j]));
        else
...
{
  if (position != pos)
  { //updates position if necessary
    position = pos;
    for (size_t i = 0; i < num_layers; i++)
    {
      layers[i].set_pos(position[i]);
    }
  }
  ...
  { //for every layer (except last one)
    ...
    { //for every neuron in said layer
      ...
      for (size_t k = 0; k < layers[i + 1].get_num_neurons(); k++)
      {
        ...
        ofDrawLine(
            float(layers[i].get_neuron(j).get_pos().x),
            float(layers[i].get_neuron(j).get_pos().y),
            float(layers[i + 1].get_neuron(k).get_pos().x),
            float(layers[i + 1].get_neuron(k).get_pos().y));
      }
    }
  }
  //draw all the layers
