...
{
  using_threads = true;
  std::cout << "Now using Multithreading\n";
}
void convneuralnet::use_single()
{
  using_threads = false;
  std::cout << "Now using a Single Core\n";
}
void convneuralnet::begin_train()
{
  ...
  for (size_t i = 0; i < num_iter; i++)
  {
    ...
    { //every 10th
      std::cout << ".";
      std::cout.flush(); //dosent affect performance much
    }
  }
...
void convneuralnet::init()
{
  ...
  for (size_t i = 0; i < num_networks; i++)
  {
    ...
    for (size_t j = 0; j < network[i].get_num_layers() - 2; j++)
    {                                 //for all layers in network[i] (not output)
    ...
    vec2 p{                                                         //position for OUTPUT layer (1 neuron ~centered)
           ...
           (num_networks / 2.0) * y_scale + 75 * i};                //divides evenly among the ypos based off number of neurons
...
{
  std::cout << "Inputting new data:" << endl;
  std::cout << "Testing on line " << network[0].get_data_line() << " out of " << total_data.size() << endl;
  for (size_t i = 0; i < num_networks; i++)
  {
    //data line indicates which line of data is being read
    //i indicates network and focus variable
    std::cout << " Network " << i << ": " << total_data[network[i].get_data_line()][i] << endl;
    ...
    if (expected_output != 0)
    {
      ...
      std::cout << "   Got: " << actual_output << " instead of "
...
#define MAX_LINE 500
  ...
  std::cout << "Saving current Network to \"" << file_at << "\"\n  ";
...
void convneuralnet::resize(vec2 init, vec2 current)
{ //compares current window to initial
  for (size_t i = 0; i < num_networks; i++)
  {
