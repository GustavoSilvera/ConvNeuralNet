...
void layer::set_weights(std::vector<double> &values)
{ //default parameter
...
void layer::update_weights(std::vector<double> &values, std::vector<double> &biases)
{ //default parameter
  //update weights of layer's neurons with new values
  ...
  for (size_t i = 0; i < num_neurons; i++)
  {
    n[i].set_weight(values[i]);
    if (i < bias_size)
      n[i].border_color(biases[i]); //safety check
  }
}
void layer::draw()
{
