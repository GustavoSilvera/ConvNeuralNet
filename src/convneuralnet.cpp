...
void convneuralnet::init(){
  ...
  for(net &n : network){//print neurons
    ...
    for(size_t j = 0; j < n.get_num_layers(); j++){
      cout << "--" << n.get_layer(j).get_num_neurons();//why seg fault??
    }
    cout << "\n";
    i++;
  }
...
#define MAX_LINE 100
  ifstream file("../src/DATA_good.txt");//using CWD
  ifstream file2("../src/DATA_good.txt");//need this to find "file_length" bc the istreambuf iterator DELETES the file after reading??
  ...
  if(file.is_open()){
    ...
    cout << "Unable to open file" << getenv("CWD");
    return false;
  }
  file.close();
  return true;
